...
//   const [formData, setFormData] = useState<FormData>({} as FormData);
  ...
  const fetchData = async () => {
    try {
      ...
      // Assuming setData, setData2, and setData3 are defined somewhere in your component
  ...
  return (
   ...
   <button type="submit" className="bg-red-600 text-white px-4 py-2 rounded-md" disabled={loading}  onClick={handleButtonClick} >
          ...
          <div>
            {activeTab === 'tab1' &&
              <div>
                ...
                <form ref={form1Ref} id="modelParametersForm" className="text-gray-900" onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 ">
                <div className="col-span-1 md:col-span-2 lg:col-span-3">
                  <div className="grid grid-cols-2 gap-4">
                    <ParameterInputField label="Dn0(Pa)" id="dn0" name="dn0" icon="Δn₀" type="text" onChange={handleChange} value={formData.dn0} required />
                    <ParameterInputField label="Dt0(Pa)" id="dt0" name="dt0" icon="Δt₀" type="text" onChange={handleChange} value={formData.dt0} required />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <ParameterInputField label="μc" id="mu" name="mu" icon="μ" type="text" onChange={handleChange} value={formData.mu} required />
                    <ParameterInputField label="ec0" id="ec0" name="ec0" icon="e₀" type="text" onChange={handleChange} value={formData.ec0} required />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <ParameterInputField label="lambda" id="lambda" name="lambda" icon="λ" type="text" onChange={handleChange} value={formData.lambda} required />
                    <ParameterInputField label="Kd" id="kd" name="kd" icon="Kd" type="text" onChange={handleChange} value={formData.kd} required />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <ParameterInputField label="A0" id="a0" name="a0" icon="A₀" type="text" onChange={handleChange} value={formData.a0} required />
                    <ParameterInputField label="Kf" id="kf" name="kf" icon="Kf" type="text" onChange={handleChange} value={formData.kf} required />
                  </div>
                </div>
                <div className="col-span-1 md:col-span-2 lg:col-span-3">
                  <div className="grid grid-cols-2 gap-4">
                    <ParameterInputField label="Kp0" id="kp0" name="kp0" icon="Kp₀" type="text" onChange={handleChange} value={formData.kp0} required />
                    <ParameterInputField label="m" id="m" name="m" icon="m" type="text" onChange={handleChange} value={formData.m} required />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <ParameterInputField label="br1(Pa)" id="br1" name="br1" icon="br₁" type="text" onChange={handleChange} value={formData.br1} required />
                    <ParameterInputField label="br2" id="br2" name="br2" icon="br₂" type="text" onChange={handleChange} value={formData.br2} required />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <ParameterInputField label="Patm(Pa)" id="patm" name="patm" icon="Pₐₜₘ" type="text" onChange={handleChange} value={formData.patm} required />
                    <ParameterInputField label="Thickness(m)" id="thickness" name="thickness" icon="t" type="text" onChange={handleChange} value={formData.thickness} required />
                  </div>
                </div>
              </div>
            </form>
  ...
  {activeTab === 'tab2' &&
              <div>
                Loading Input
                <form ref={form2Ref} id="loadingInputForm" className="text-gray-900" onSubmit={handleSubmit}>
              <div className="grid grid-cols-2 gap-4">
                <ParameterInputField label="Tang. Disp Max (m)" id="tangDispMax" name="tangDispMax" icon="⇑" type="text" onChange={handleChange} value={formData.tangDispMax} required />
                <ParameterInputField label="Tang. Disp Min (m)" id="tangDispMin" name="tangDispMin" icon="⇓" type="text" onChange={handleChange} value={formData.tangDispMin} required />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <ParameterInputField label="Number of Cycle" id="numCycle" name="numCycle" icon="🔄" type="number" onChange={handleChange} value={formData.numCycle} required />
                <ParameterInputField label="Number of Increment" id="numIncrement" name="numIncrement" icon="➕" type="number" onChange={handleChange} value={formData.numIncrement} required />
              </div>
            </form>
                </div>
  ...
  {activeTab === 'tab3' && (
              <div>
                Initial Condition
                <form ref={form3Ref} id="initialConditionForm" className="text-gray-900" onSubmit={handleSubmit}>
              <div className="grid grid-cols-2 gap-4">
                <ParameterInputField label="Sigma-n0 (Pa)" id="sigmaN0" name="sigmaN0" icon="σ" type="text" onChange={handleChange} value={formData.sigmaN0} required />
                <ParameterInputField label="Initial Void Ratio (e0)" id="initialVoidRatio" name="initialVoidRatio" icon="e₀" type="text" onChange={handleChange} value={formData.initialVoidRatio} required />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <ParameterInputField label="Normal Stiffness (Pa/m)=-K" id="normalStiffness" name="normalStiffness" icon="K" type="text" onChange={handleChange} value={formData.normalStiffness} required />
              </div>
            </form>
                </div>
          ...
          />
                                      ...
                                      <div className="mt-2">
                                          ...
                                          {showGraph1 ? (
                                              ...
                                              <GraphPage loader={startLoader}  key={reloadCyclicG.toString()} ></GraphPage>
